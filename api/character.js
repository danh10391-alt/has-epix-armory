// Has Epix Armory — Vercel Serverless API (CommonJS)
const fs   = require('fs');
...
let _token = null;
let _tokenExpires = 0;
const _iconDataCache  = new Map();
const _itemIconCache  = new Map();
const _itemSocketCache= new Map();
const _itemLevelCache = new Map();
...
// ── fetch wrapper (Node 18 global fetch) ───────────────────────────────────────
...
async function getToken() {
  ...
  const resp = await apiFetch(TOKEN_URL, {
    method: 'POST',
...
async function wowApi(urlPath, namespace, token, extra = {}) {
  ...
  try {
    const resp = await apiFetch(`${BASE_URL}${urlPath}?${params}`, {
...
async function fetchItemStaticData(itemId, token) {
  ...
  if (socCached && lvlCached) return [_itemSocketCache.get(itemId), _itemLevelCache.get(itemId)];
  try {
    ...
    return [[], _itemLevelCache.get(itemId) || null];
  }
}
  ...
  try {
    ...
    const [iconResults, staticResults] = await Promise.all([
      Promise.all(itemIds.map(async id => {
        const iname = await getItemIconName(id, token);
        const idata = iname ? await fetchIconDataUri(iname, 'large') : null;
        return [id, iname, idata];
      })),
      Promise.all(itemIds.map(async id => {
