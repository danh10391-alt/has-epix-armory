...
// Load talent positions data
let TALENT_POSITIONS = null;
try {
  TALENT_POSITIONS = JSON.parse(fs.readFileSync(path.join(__dirname, 'talent_positions.json'), 'utf8'));
} catch (e) {
  console.error('Failed to load talent_positions.json:', e.message);
}
...
async function buildEquipment(equippedItems, token) {
  ...
  await Promise.all(equippedItems.map(async (item) => {
    ...
    // Sockets — prefer preview_item.sockets (has ALL slots, empty + filled)
    // Fall back to static data socket_types if preview_item missing
    const previewSockets = (item.preview_item && item.preview_item.sockets)
      ? item.preview_item.sockets
      : (staticData ? staticData.socket_types.map(t => ({ socket_type: { type: t } })) : []);

    const sockets = [];
    for (let si = 0; si < previewSockets.length; si++) {
      const ps = previewSockets[si];
      const sockType = (ps.socket_type && ps.socket_type.type) || 'PRISMATIC';

      // Gem may be in preview_item.sockets[si].item OR item.sockets[si].item
      const psGem = ps.item || null;
      const eqGem = (item.sockets && item.sockets[si]) ? (item.sockets[si].item || null) : null;
      const gemItem = psGem || eqGem;
      const gemStats = ps.display_string || (item.sockets && item.sockets[si] && item.sockets[si].display_string) || null;

      // Fetch gem icon as base64 (same pipeline as item icons)
      let gemIconData = null;
      let gemIconName = null;
      if (gemItem && gemItem.id) {
        const gmUrl = blizzardUrl(`/data/wow/media/item/${gemItem.id}`, STATIC_NS);
        const { data: gmData } = await blizzGet(gmUrl, token).catch(() => ({ data: null }));
        if (gmData && gmData.assets) {
          const gmIconUrl = (gmData.assets.find(a => a.key === 'icon') || {}).value;
          if (gmIconUrl) {
            const m2 = gmIconUrl.match(/\/([^\/]+?)(?:\.jpg)?$/i);
            if (m2) {
              gemIconName = m2[1].toLowerCase();
              gemIconData = await fetchIconBase64(gemIconName);
            }
          }
        }
      }
      ...
      sockets.push({
        socket_type: sockType,           // 'RED', 'BLUE', 'YELLOW', 'META', 'PRISMATIC'
        gem_name: gemItem ? (gemItem.name || null) : null,
        gem_stats: gemStats,
        gem_icon: gemIconName,
        gem_icon_data: gemIconData,
      });
    }
...
// ── Build talents (spec points + per-talent ranks + tree layout) ──────────────
function buildTalents(charData, specData) {
  ...
  const per_talent_ranks = {};  // spell_id (string) -> rank spent
  ...
  try {
    ...
    if (activeGroup && activeGroup.specializations) {
      activeGroup.specializations.slice(0, 3).forEach((spec, i) => {
        ...
        // Extract individual talent selections
        // API returns spec.talents[], each with spell_tooltip.spell.id (spell_id) and talent_rank
        const talentList = spec.talents || spec.spec_talent_list || [];
        talentList.forEach(entry => {
          // Use spell_id (from spell_tooltip.spell.id) — this matches talent_positions.json spell_ids
          const spellInfo = entry.spell_tooltip && entry.spell_tooltip.spell;
          const spellId = spellInfo ? spellInfo.id : (entry.talent ? entry.talent.id : null);
          const rank = entry.talent_rank || 0;
          if (spellId && rank > 0) {
            per_talent_ranks[String(spellId)] = rank;
          }
          // Also index by name for fallback
          const spellName = spellInfo ? spellInfo.name : null;
          if (spellName && rank > 0) {
            per_talent_ranks['name:' + spellName.toLowerCase().trim()] = rank;
          }
        });
      });
    }
  ...
  // Include the talent tree layout for this class (from talent_positions.json)
  let talent_trees = [];
  if (classId && TALENT_POSITIONS && TALENT_POSITIONS.by_class_id) {
    talent_trees = TALENT_POSITIONS.by_class_id[String(classId)] || [];
  }

  return { trees, spec_summary, total_points, tree_names: treeNames, per_talent_ranks, talent_trees };
}
